/*==============================================================================
  ...
  Programme:   Bachelor in Economics, IE University
  ...
  in the thesis. It is organised in three parts:
      ...
      Part A -- Core Bitcoin Analysis (Tables 1-8, Figures 1-7)
      Part B -- Gold Benchmark Analysis (Figures 14-16)
      Part C -- Geopolitical Event Study (Tables 9-10, Figures 8-13)

  ------------------------------------------------------------------------------
  WORKING DIRECTORY
  ------------------------------------------------------------------------------
  C:\Users\mnazer.ieu2022\Documents\

  ------------------------------------------------------------------------------
  DATA SOURCES (all placed in working directory)
  ------------------------------------------------------------------------------
      Bitcoin_Historical_Data__4_.csv   Bitcoin USD daily, via CoinMarketCap
      XAU_USD_Historical_Data.csv       Gold XAU/USD daily, via Investing.com
      S_P_500_Historical_Data__2_.csv   S&P 500 daily, via Investing.com

  ------------------------------------------------------------------------------
  REQUIREMENTS
  ------------------------------------------------------------------------------
  Software:  Stata 16 or later
  Packages:  estout (installed automatically below)

  ------------------------------------------------------------------------------
  TABLE MAPPING (as reported in thesis body)
  ------------------------------------------------------------------------------
      Table 1   Full-sample descriptive statistics
      Table 2   Descriptive statistics by regime
      Table 3   Baseline return regressions
      Table 4   ETF event-window return regression
      Table 5   Volatility regression (squared returns)
      Table 6   Tail-risk frequencies by regime
      Table 7   Peak-to-trough drawdown analysis
      Table 8   GARCH(1,1)-t model estimates across specifications  [NEW]
      Table 9   60-day returns after geopolitical events
      Table 10  Immediate impact of geopolitical events
      Table 11  Survey response summary (external -- not produced here)
==============================================================================*/
...
* Regime definitions
*   Regime 1: 2016-01-02 to 2020-12-31  (baseline; regime2==0 & regime3==0)
*   Regime 2: 2021-01-01 to 2023-12-31
*   Regime 3: 2024-01-01 to 2026-01-01
gen regime2 = inrange(date, td(01jan2021), td(31dec2023))
gen regime3 = inrange(date, td(01jan2024), td(01jan2026))

* ETF event windows (central event: 10 January 2024 SEC approval)
...
* Analysis sample: non-missing returns and regime indicators, bounded at 01jan2026
...
esttab using "Table1_Descriptives.rtf", replace ///
    ...
    title("Table 1. Full-Sample Descriptive Statistics")
...
esttab reg1 reg2 reg3 using "Table2_ByRegime.rtf", replace ///
    cells("mean sd") ///
    mtitles("2016-2020" "2021-2023" "2024-2026") ///
    title("Table 2. Descriptive Statistics by Regime")
...
esttab m1 m2 using "Table3_Returns.rtf", replace ///
    ...
    title("Table 3. Baseline Return Regressions (Robust SE)")

*===============================================================================
* A6. TABLE 4 -- ETF Event-Window Return Regression
*  (Reports in Section 5.3 of thesis; appears in text before volatility results.)
*===============================================================================
...
esttab etf1 etf2 using "Table4_ETF_Event.rtf", replace ///
    se star(* 0.10 ** 0.05 *** 0.01) ///
    title("Table 4. ETF Event-Window Return Regression (Robust SE)")

* Appendix robustness: 1-day / 3-day / 7-day event windows
eststo clear
...
esttab w1 w3 w7 using "AppendixTable_ETF_Windows.rtf", replace ///
    se star(* 0.10 ** 0.05 *** 0.01) ///
    title("Appendix Table. ETF Event-Window Robustness (1-day, 3-day, 7-day)")

*===============================================================================
* A7. TABLE 5 -- Volatility Regression (Squared Returns)
*  (Reports in Section 5.4 of thesis.)
*===============================================================================
...
esttab v1 using "Table5_Volatility.rtf", replace ///
    se star(* 0.10 ** 0.05 *** 0.01) ///
    title("Table 5. Volatility Regression - Squared Returns (Robust SE)")

*===============================================================================
* A8. TABLE 6 -- Tail-Risk Frequencies by Regime
*===============================================================================
...
*===============================================================================
* A9. TABLE 7 -- Drawdown Analysis by Regime
*===============================================================================
...
* Max and mean drawdown by regime
preserve
...
* Percentage of days spent in drawdown (supplementary)
...
*===============================================================================
* A10. ARCH-LM TEST AND TABLE 8 -- GARCH(1,1)-t ACROSS THREE SPECIFICATIONS
*
*  This section produces the three-specification comparison reported in Table 8
*  of the thesis (Section 5.5):
*     (1) Full-sample unconstrained GARCH(1,1)-t
*     (2) IGARCH(1,1)-t with alpha + beta = 1 imposed
*     (3) GARCH(1,1)-t on the 2024-2026 sub-sample only
*
*  The robust-SE version is estimated as a supplementary check but not tabled.
*===============================================================================

* ARCH-LM test on baseline return equation
reg ret if sample_main==1
predict resid, resid
estat archlm

* ----- Specification (1): Full-sample unconstrained GARCH(1,1)-t -----
arch ret if sample_main==1, arch(1) garch(1) distribution(t)
estimates store full_garch

* Wald test: is alpha + beta statistically distinguishable from 1?
test [ARCH]L1.arch + [ARCH]L1.garch = 1

* Predicted conditional volatility for Figure 3
predict h_t, variance
gen vol_garch = sqrt(h_t)

* Supplementary: same model with semi-robust (OPG) standard errors
arch ret if sample_main==1, arch(1) garch(1) distribution(t) vce(robust)
estimates store full_robust

* ----- Specification (2): IGARCH(1,1)-t (alpha + beta = 1 constrained) -----
constraint 1 [ARCH]L1.arch + [ARCH]L1.garch = 1
arch ret if sample_main==1, arch(1) garch(1) distribution(t) constraints(1)
estimates store igarch

* ----- Specification (3): Regime 3 sub-sample only -----
arch ret if sample_main==1 & regime3==1, arch(1) garch(1) distribution(t)
estimates store r3_only

* ----- Export Table 8 comparison -----
esttab full_garch igarch r3_only using "Table8_GARCH_Comparison.rtf", replace ///
    b(%9.4f) se(%9.4f) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    stats(N ll aic bic, fmt(0 2 2 2) ///
        labels("Observations" "Log-likelihood" "AIC" "BIC")) ///
    mtitles("(1) Full Sample" "(2) IGARCH" "(3) Regime 3 only") ///
    title("Table 8. GARCH(1,1)-t Model Estimates Across Specifications") ///
    note("Standard errors in parentheses. * p<0.10, ** p<0.05, *** p<0.01.")

* Also export a wide-format comparison table (more readable for appendix)
estimates table full_garch full_robust igarch r3_only, ///
    b(%9.4f) se stats(N ll aic bic)

*===============================================================================
* A11. FIGURES 1-4 -- Core Time-Series Plots
*===============================================================================
...
* Figure 3: GARCH(1,1) Conditional Volatility (full-sample model)
tsline vol_garch if sample_main==1, ///
    ytitle("GARCH Conditional Volatility") xtitle("Date") ///
    xlabel(`=td(01jan2016)' "2016" `=td(01jan2018)' "2018" `=td(01jan2020)' "2020" `=td(01jan2022)' "2022" `=td(01jan2024)' "2024" `=td(01jan2026)' "2026", labsize(small)) ///
    graphregion(margin(r=4)) ///
    note("Full-sample unconstrained GARCH(1,1)-t; early periods interpreted with caution (see Section 5.5).", size(vsmall))
...
*===============================================================================
* A12. FIGURES 5-6 -- Rolling Volatility and Rolling Tail-Risk Frequency
*===============================================================================
...
*===============================================================================
* A13. FIGURE 7 -- ETF Event-Window Price Chart
*===============================================================================
...
*===============================================================================
* B5. FIGURES 14-15 -- Gold Price and Rolling Volatility
*===============================================================================

* Figure 14: Gold Price Over Time
...
graph export "Figure14_GoldPrice.png", replace width(1200)

* Figure 15: Gold 30-Day Rolling Volatility
gen gold_roll_sd = .
gen t_gold = _n
...
graph export "Figure15_GoldRollingVolatility.png", replace width(1200)


*###############################################################################
*  PART C: GEOPOLITICAL EVENT STUDY (THREE-ASSET COMPARISON)
*
*  This part merges Bitcoin, gold, and S&P 500 onto a common calendar.
*  The resulting Event_Merged.dta is reused in Section B6 below for the
*  rolling BTC-gold correlation (Figure 16).
*###############################################################################
...
*===============================================================================
* B6. FIGURE 16 -- 90-Day Rolling Correlation: Bitcoin vs Gold
*
*  NOTE ON PLACEMENT:
*  This section is physically located in Part C of the do-file because it
*  reuses the Event_Merged.dta produced in C1. Logically, it belongs to the
*  gold-benchmark analysis (Part B) and is referenced in thesis Section 5.8.1.
*  Figure 16 complements the descriptive gold comparison with continuous
*  time-varying correlation evidence.
*===============================================================================

use "Event_Merged.dta", clear

* Keep only trading days where both BTC and gold returns exist
keep if !missing(btc_ret) & !missing(gold_ret)
sort date
gen tid = _n

* Manual rolling 90-day Pearson correlation
gen roll_corr = .
local N = _N
forvalues i = 90/`N' {
    local start = `i' - 89
    quietly corr btc_ret gold_ret in `start'/`i'
    replace roll_corr = r(rho) in `i'
}

format date %td

twoway (line roll_corr date if !missing(roll_corr), lcolor(navy) lwidth(medium)), ///
    yline(0, lcolor(black) lpattern(dash) lwidth(thin)) ///
    title("90-Day Rolling Correlation: Bitcoin vs Gold Returns", size(medium)) ///
    ytitle("90-Day Rolling Correlation") xtitle("Date") ///
    xlabel(`=td(01jan2016)' "2016" `=td(01jan2018)' "2018" `=td(01jan2020)' "2020" `=td(01jan2022)' "2022" `=td(01jan2024)' "2024" `=td(01jan2026)' "2026", labsize(small)) ///
    graphregion(margin(r=4)) ///
    note("Source: Author's calculations from CoinMarketCap (Bitcoin) and Investing.com (XAU/USD).", size(vsmall))
graph export "Figure16_RollingCorrelation.png", replace width(1200)

* Summary statistics on the rolling correlation series (for thesis text)
summ roll_corr if !missing(roll_corr), detail
...
*===============================================================================
* C4. EXPORT TABLES 9 AND 10
*===============================================================================

* --- Table 9: 60-day returns ---
tempname fh
...
*===============================================================================
* C6. FIGURES 10-13 -- Indexed Price Path Graphs Around Individual Events
*===============================================================================
...
di as text "{hline 80}"
di as text ""
di as text "  MAIN TABLES (thesis body):"
di as text "    Table1_Descriptives.rtf"
di as text "    Table2_ByRegime.rtf"
di as text "    Table3_Returns.rtf"
di as text "    Table4_ETF_Event.rtf"
di as text "    Table5_Volatility.rtf"
di as text "    Table6_TailRisk.csv"
di as text "    Table7_Drawdown.csv + Table7b_DrawdownDays.csv"
di as text "    Table8_GARCH_Comparison.rtf          [NEW]"
di as text "    Table9_Event60d.rtf"
di as text "    Table10_EventImmediate.rtf"
di as text ""
di as text "  APPENDIX TABLES:"
di as text "    AppendixTable_ETF_Windows.rtf"
di as text ""
di as text "  FIGURES:"
...
di as text "    Figure14_GoldPrice.png"
di as text "    Figure15_GoldRollingVolatility.png"
di as text "    Figure16_RollingCorrelation.png     [NEW]"
di as text ""
